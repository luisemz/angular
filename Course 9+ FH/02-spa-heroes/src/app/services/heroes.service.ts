...
export class HeroesService {
  ...
  getHeroes = (): Heroe[] => this.hereos;

  getHeroe = (id: number): Heroe =>
    id >= 0 && id < this.hereos.length
      ? this.hereos.find(heroe => {
          return heroe.id === Number(id);
        })
      : null;

  searchsHeroe = (text: string): Heroe[] => {
    let heroesArr: Heroe[] = [];
    text = text.toLowerCase();

    for (let heroe of this.hereos) {
      let name = heroe.nombre.toLowerCase();
      if (name.indexOf(text) >= 0) heroesArr.push(heroe);
    }
